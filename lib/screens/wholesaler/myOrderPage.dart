   ...
   final userProfile = item.callback != null && item.callback!.isNotEmpty
    ...
    final email = userProfile != null
        ...
        : 'No Email';
                                       ...
                                       //  wholereviewcontroller.reviewAdd(
                                       //    item.id ?? 0,
                                       //    (  wholemyordercontroller.wholeorderdetailsModel!.data![0].orderId ?? 0));
                                      wholereviewcontroller.reviewAdd(
                                          item.callback![0].itemId ?? 0, // Assuming itemId is of type int
                                          int.tryParse((item.id ?? '').toString()) ?? 0 // Convert to string and then parse as int
                                      );
                                      ...
                                      //
                                      //     ));
                                      print("item total ${item.callback![0].totalAddOnPrice}");
                                      var total_add_on=item.callback![0].totalAddOnPrice.toString();
                                      Get.to(OrderDetailswhole(
                                        orderId: item.id??0,
                                        couponcode: item.couponCode ?? '',
                                        paymentmethod: item.paymentMethod ?? '',
                                        ...
                                        phone: phone,
                                        email: email,
                                        itemPrice: item.callback![0]?.price ?? '',
                                        taxAmount: item.callback![0]?.taxAmount ?? '',
                                        itemTotal:total_add_on ??'',
                                        variant: item.callback![0]?.variation??'',
                                        itemQuantity: item.callback![0].quantity.toString()??'',
                                        variation: item.callback![0].variation??'',
                                        //gst: item.gst??'',
                                        address: item.deliveryAddress ?? '',
                                        delivered: item.delivered ?? '',
                                        deliverycharge: item?.deliveryCharge ?? '',
