...
import 'package:canine/models/wholesalerModel/ProductDetailsWholeModel.dart'as variantFile;
import 'package:canine/models/wholesalerModel/mycartListModel.dart';
...
// var selectedVariants ;
  ...
  void  selectImageUpate( ){
    ...
    imagesPath = "${Constants.BASE_URL}/storage/app/public/product/${productdetailwholemodel!.data!.images??''}";
    imagePath = "${Constants.BASE_URL}/storage/app/public/product/${productdetailwholemodel!.data!.images![selectImages??0].toString()}";
  ...
  void onInit() {
    ...
    print("WholeSaler Id: ${wholesalerID}");
  }
...
//  bool isAdding = false;
  ...
  void  allamount(){
    totalAmount = ((selectedvariants?.price ?? 0) * (sizecount ?? 0) - (((selectedvariants?.price ?? 0) * sizecount ?? 0) * (productdetailwholemodel!.data!.discount!) / 100));
    // print("-------"+totalAmount);
  }
...
// clearFields();
  ...
  Future<void> ReOrderProduct() async {
    ...
    try {
      ...
      request.fields.addAll({
        ...
        "quantity":orderReOrder!.quantity.toString(),
        "image": orderReOrder!.itemDetails![0].image.toString(),
        "price": (double.parse(orderReOrder!.price!)/(orderReOrder!.quantity ??0)).toString(),
        "total_quantity":0.toString(),
        "return_order":'Yes',
      });
  ...
  Future<void> addProductHome() async {
    ...
    var body = {
      ...
      "total_quantity":1,
      // "total_quantity":selectedvariants?.stock.toString()??0.0,
  ...
  Future<void> addProduct() async {
    ...
    var body = {
      ...
      "price":  (selectedvariants?.wholeprice != null)? (((selectedvariants?.wholeprice ?? 0)  -
          (((selectedvariants?.wholeprice ?? 0) ) * (productdetailwholemodel!.data!.discount!) / 100))).toString():(((productdetailwholemodel!.data!.wholePrice ?? 0)  -
          (((productdetailwholemodel!.data!.wholePrice ?? 0)) * (productdetailwholemodel!.data!.discount!) / 100))).toString(),
    ...
    try {
      ...
      request.fields.addAll({
        ...
        "quantity": (sizecount ?? 0).toString(),
        "image": productdetailwholemodel!.data!.image ?? '',
        "price": (selectedvariants?.wholeprice != null)? (((selectedvariants?.wholeprice ?? 0)  - (((selectedvariants?.wholeprice ?? 0)) *
            (productdetailwholemodel!.data!.discount!) / 100))).toString():(((productdetailwholemodel!.data!.wholePrice ?? 0)  -
            (((productdetailwholemodel!.data!.wholePrice ?? 0) ) * (productdetailwholemodel!.data!.discount!) / 100))).toString(),
        //  ((selectedvariants?.wholeprice ?? 0) * (sizecount ?? 0) -
        //         (((selectedvariants?.wholeprice ?? 0) * sizecount ?? 0) *
        //             (productdetailwholemodel!.data!.discount!) /
